          ...
          <v-toolbar flat color="white">
            ...
            <div class="printBttn">
              <button v-print="'#printMe'">
                <i class="material-icons">print</i>{{ $t('print') }}
              </button>
            </div>
          </v-toolbar>
            ...
            <div class="col s10">
              <div class="col s10">
                ...
                <div v-if="dateError" class="error-message">
                  <span class="error-text">{{ $t('dueDateError') }}</span>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  mounted() {
    ...
    paramVal.start_date = this.startDate;
    paramVal.end_date = this.endDate;
    this.$http
  ...
  watch: {
    startDate(currentdate) {
      ...
      if (endDate) {
        if (moment(endDate).isSameOrAfter(currentdate, "day")) {
          ...
          this.filterData(currentdate, endDate);
        } else {
          this.dateError = true;
          this.startDate = null;
        }
    ...
    endDate(currentdate) {
      ...
      if (startDate) {
        if (moment(currentdate).isSameOrAfter(startDate, "day")) {
          this.dateError = false;
          this.filterData(startDate, currentdate);
        } else {
          this.dateError = true;
          this.endDate = null;
        }
