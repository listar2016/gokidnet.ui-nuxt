          ...
          <div class>
            <div class="row">
              <div class="col s3 left">
                <h5 style="padding: 16px 0 0 24px;">{{ $t('billings.editPlan') }}</h5>
              </div>
...
export default {
  ...
  mounted() {
    this.$store.commit("user/setTitle", this.$t('billings.editPlan'));
    ...
    this.$http
      .get("/billing_recurring_plans/" + this.$route.params.id)
      .then(response => {
  ...
  methods: {
    ...
    sendInvoice() {
      this.$http
        .put(`/billing_recurring_plans/${this.$route.params.id}/`, this.invoice)
        .then(response => {
