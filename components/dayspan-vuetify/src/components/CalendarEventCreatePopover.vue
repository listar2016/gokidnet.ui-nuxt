    ...
    <v-card-text>
      <v-container grid-list-md>
        <div class="row" @keyup.enter="save" v-if="calendar.placeholderForCreate===true || true">
          <div class="row">
            <div class="col s8">
              <MyInput name="Title" :error="error" :value="newItem" prop-name="title" />
            </div>
            <div class="col s4">
              <v-checkbox v-model="newItem.is_public" label="Is public?"></v-checkbox>
            </div>
          </div>
          <Select
            class="col m12"
            ...
            :multiple="true"
          />
          <v-textarea class="col m12" v-model="newItem.notes" label="Notes"></v-textarea>
          <div class="row">
            <div class="col m6">
              <MyDate
                v-if="test"
...
export default {
  ...
  data: vm => ({
    ...
    newItem: {
      ...
      is_public: true,
      rooms: [],
      plan: 2 // STATIC, TEMPORARY WHILE BACK IS NOT WORKING
    },
    test: false,
    oldInterval: null
  }),
  ...
  methods: {
    kostil() {
      this.test = false;
      var startDate = moment(this.calendarEvent.start.time).format('YYYY-MM-DD');
      var endDate = moment(this.calendarEvent.end.time).format('YYYY-MM-DD');
      var startTime = moment(this.calendarEvent.start.time).format('HH:mm');
      var endTime = moment(this.calendarEvent.end.time).format('HH:mm');
      ...
      this.newItem = {
        ...
        endDate: endDate,
        day:
          days[new Date(this.calendarEvent.start.time + 5 * 3600000).getDay()],
        plan: 2
      };
    ...
    save() {
      ...
      EventBus.$emit("update-events", this.newItem);
