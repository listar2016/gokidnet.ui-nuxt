...
export default {
  ...
  props:
  {
    ...
    types:
    {
      type: Array,
      default() {
        return this.$dsDefaults().types.filter((x) => {
          return ['Week', 'Month', 'Day'].indexOf(x.label) !== -1
        });
      }
    },
    allowsAddToday:
  ...
  methods:
  {
    ...
    addAt(dayHour)
    {
      ...
      let calendar = this.$refs.calendar;
      let useDialog = !this.hasCreatePopover;
      dayHour.day.hour = parseInt(dayHour.hour.split(':')[0])
      dayHour.day.minute = parseInt(dayHour.hour.split(':')[1])
      let at = dayHour.day.withTimes(parseInt(dayHour.hour.split(':')[0] ), parseInt(dayHour.hour.split(':')[1]), 0, 0)
      // at.minute = parseInt(dayHour.hour.split(':')[1])
    ...
    handleMove(moveEvent)
    {
      ...
      if (schedule.isSingleEvent())
      {
        ...
        if (this.$dayspan.features.moveDuplicate && false)
        {
          options.push({
